...
export async function removeVoucherFromBooking(
  ...
  const response = await requestApi<unknown>(
    `/api/bookings/${encodeURIComponent(bookingCode.trim())}/voucher`,
    {
      accessToken,
      fallbackMessage: "Không thể bỏ voucher khỏi booking này lúc này.",
      method: "DELETE"
    }
  );

  if (!isManageOverview(response)) {
    throw new ApiClientError("Dữ liệu bỏ voucher trả về không hợp lệ.", 500);
  }
